...
Util.easyDate = function(date) {
  ...
  if (delta < 120) {
    return prefix + '1minuto';
  }
  if (delta < 3300) {
    min = parseInt(delta / 60, 10);
    return prefix + min + 'minutos';
  }
  if (delta < 7200) {
    return prefix + '1h';
  }
...
Annotator.Erratum = (function(_super) {
  ...
  Erratum.prototype._setupComment = function(annotation, onload) {
    ...
    replies = annotation.replies || [];
    replies = []; // Retirada da funcionalidade de resposta (Reply)
    if (replies.length) {
      $('<dt class="erratum-header-replies">Respostas</dt>').appendTo(erratum);
    }
